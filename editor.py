...
현재 에디터는 기존 "챕터" 단위로 분기를 편집하던 구조를 유지합니다.
게임 엔진은 "챕터" 안에 여러 "분기"(branch)를 포함하는 형식으로 변경되었으나,
에디터에서는 아직 해당 구조를 완전히 지원하지 않습니다.
향후 업데이트에서는 챕터 안에 분기를 편집할 수 있도록 개선될 예정입니다.
